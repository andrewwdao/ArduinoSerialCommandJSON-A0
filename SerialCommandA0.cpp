...
uint8_t SerialCommand::available()   //ham kiem tra xem co tin hieu tu serial hay khong
{
	// If we're using the Hardware port, check it.   Otherwise check the user-created SoftwareSerial Port
	#ifdef SERIALCOMMAND_HARDWAREONLY
	if (Serial.available() > 0) 
	#else
	if ((usingSoftwareSerial==0 && Serial.available() > 0) || (usingSoftwareSerial==1 && SoftSerial->available() > 0) )
	#endif
	{
		return 1;
	}
